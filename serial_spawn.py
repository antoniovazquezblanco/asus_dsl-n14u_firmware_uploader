...
class SerialSpawn(SpawnBase):
    ...
    def read_nonblocking(self, size=1, timeout=-1):
        ...
        while self.ser.in_waiting <= 0  and time.time() < limit_time:
            time.sleep(0.1)
        n = min(size, self.ser.in_waiting)
        if n > 0:
            s = self.ser.read(n)
    ...
    def _interact_output_loop(self, output_filter=None):
    ...
    def interact(self, escape_character=chr(29), input_filter=None, output_filter=None):
        ...
        self._interact_output_loop(output_filter)
